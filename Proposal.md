...
| Shuko Takamura | 
| Ece Polat | 
*(Roles are a starting split — everyone contributes across the stack as needed.)*

## 7. Milestones

| Phase | Deliverable |
|---|---|
| 1. Discovery | Data audit of CBS alumni export; confirm API access/keys; finalize schema design |
| 2. Data pipeline | ETL script ingesting + cleaning alumni data into SQL database |
| 3. Enrichment | API integrations live; company/role data joined and stored |
| 4. Backend | REST API endpoints built and tested against the enriched database |
| 5. Frontend | Vue/TS app: search, filters, detail views wired to the backend |
| 6. QA & polish | End-to-end testing, edge cases, UI polish |
| 7. Demo | Final walkthrough / presentation |

## 8. Risks & Mitigations

- **Company name matching is messy** (e.g. "Goldman Sachs" vs "Goldman Sachs & Co."). → Build a normalization/fuzzy-matching step with manual override list.
- **API rate limits on free tiers.** → Cache enrichment results in the database; only re-fetch on a schedule, not per request.
- **Alumni data sensitivity.** → Follow CBS data-handling requirements; avoid exposing PII beyond what's needed for the demo.
- **Scope creep** given five workstreams (data, backend, frontend, APIs, UX). → Lock MVP scope after Phase 1 and treat anything beyond it as a stretch goal.

## 9. Success Criteria

- A working end-to-end demo: search/filter the alumni dataset and see enriched company + comparable-role data per result.
- A measurable enrichment coverage rate (e.g. % of employers successfully matched and enriched).
- Positive usability feedback from a small group of CBS students in an informal test session.
